...
- 節の境界は次の H2 レベル見出しのみ: grep 流に書けば `^##[^#]`
  （`###` は境界ではない）。参照実装は裸の `##` 行も数えられるよう
  `^##(?!#)` と書いている。
...
2. **ブロック内の H2 レベル見出しはちょうど1個**（フェンス対応で数える）。
   H2 が2個あるブロックは、次回の実行で隣の節を静かに吸収する。ブロック内の
...
2個目の H2・どちらか一方でも未クローズのフェンス — は推測で処理せず
終了コード 2 で停止します。
...
- 列0の ATX 見出しのみ。Setext 見出し（`見出し` + `===`/`---` の下線）、
  閉じハッシュ形式（`## X ##`）、1〜3個の空白でインデントされた見出しは
  本文として扱われます。正本節の見出しは列0の素の `## Name` 形式に保って
  ください。
- フェンス処理は CommonMark のコア規則の範囲（列0〜3のバッククォート/チルダ
  フェンス）。特殊ケース（blockquote 内や深いリストインデント内のフェンス）は
  参照実装のスコープ外です。
- UTF-8 文書のみ。改行が混在するファイルは初回の書き込みで検出された支配的
  スタイルに正規化され、2回目以降は安定します。
