...
- Reference implementation `scripts/merge_section.py` (Python 3, standard
  ...
  mode with diff-style exit codes, byte-for-byte LF/CRLF and UTF-8 BOM
  preservation.
- Fixtures for the trap case (heading inside a code fence), append,
  replace, and the `###` subheading boundary, plus the dependency-free
  self-test `scripts/test_merge_section.py` — including measured
  falsification of the fence-blind implementation (document corruption,
  non-idempotence, early cut at `###`).
...
- GitHub Actions workflow for validation, reference implementation tests,
  private-marker scanning, and whitespace checks.
