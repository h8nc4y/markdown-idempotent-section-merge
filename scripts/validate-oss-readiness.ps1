...
Assert-FileContains -RelativePath 'README.md' -Pattern '(?is)raw LF bytes.*1,000,000.*250,000.*exact.*final-output limit.*no-op.*limit-plus-one.*before.*decoding' -Description 'public newline-budget contract'
...
Assert-FileContains -RelativePath 'SKILL.md' -Pattern '(?is)230\.06 MiB.*descriptive.*peak-memory-characterization\.md.*raw LF bytes.*1,000,000.*250,000.*no streaming parser' -Description 'skill peak-memory and newline-budget contract'
Assert-FileContains -RelativePath 'docs/SKILL.ja.md' -Pattern '(?is)230\.06 MiB.*記述値.*peak-memory-characterization\.md.*raw LF.*1,000,000.*250,000.*streaming parser' -Description 'Japanese peak-memory and newline-budget contract'
...
Assert-FileContains -RelativePath 'SECURITY.md' -Pattern '(?is)raw LF-byte limits.*1,000,000.*250,000.*before.*UTF-8 decoding.*before temporary creation.*230\.06 MiB.*descriptive.*not a cross-platform.*peak-memory-characterization\.md.*newline budget' -Description 'security newline-budget and peak-memory contract'
Assert-FileContains -RelativePath 'docs/peak-memory-characterization.md' -Pattern '(?is)5 cases.*3 repetitions.*process lifetime.*peak RSS.*230\.06 MiB.*LINE-BUDGET-01.*1,000,000.*250,000.*streaming parser' -Description 'historical peak-memory evidence and implemented newline boundary'
...
Assert-FileContains -RelativePath 'scripts/merge_section.py' -Pattern '_MAX_OUTPUT_BYTES\s*=\s*_MAX_TARGET_BYTES' -Description 'output limit closes over the target input limit'
Assert-FileContains -RelativePath 'scripts/merge_section.py' -Pattern 'if len\(out_bytes\) > _MAX_OUTPUT_BYTES:' -Description 'final raw output byte guard'
Assert-FileContains -RelativePath 'scripts/merge_section.py' -Pattern '_OUTPUT_OVERSIZE_ERROR\s*=\s*"merged output exceeds the supported byte limit"' -Description 'fixed output oversize diagnostic'
Assert-FileContains -RelativePath 'scripts/merge_section.py' -Pattern '(?is)_MAX_TARGET_NEWLINES\s*=\s*1_000_000.*_MAX_BLOCK_NEWLINES\s*=\s*250_000.*_MAX_OUTPUT_NEWLINES\s*=\s*_MAX_TARGET_NEWLINES' -Description 'target block and output newline limits'
Assert-FileContains -RelativePath 'scripts/merge_section.py' -Pattern '(?is)def _assert_raw_newline_budget.*raw\.count\(b"\\n"\).*raise MergeError\(oversize_error\)' -Description 'raw LF-byte fail-closed helper'
Assert-FileContains -RelativePath 'scripts/merge_section.py' -Pattern '(?is)_TARGET_NEWLINE_OVERSIZE_ERROR\s*=.*"target exceeds the supported newline count limit".*_BLOCK_NEWLINE_OVERSIZE_ERROR\s*=\s*"block exceeds the supported newline count limit".*_OUTPUT_NEWLINE_OVERSIZE_ERROR\s*=.*"merged output exceeds the supported newline count limit"' -Description 'fixed path-free newline diagnostics'
Assert-FileContains -RelativePath 'scripts/merge_section.py' -Pattern '(?is)_assert_raw_newline_budget\(\s*raw,.*?_decode\(raw, "target"\).*?_assert_raw_newline_budget\(\s*block_bytes,.*?_decode\(block_bytes, "block"\).*?if len\(out_bytes\) > _MAX_OUTPUT_BYTES:.*?_assert_raw_newline_budget\(\s*out_bytes,' -Description 'newline checks precede input decode and temporary commit path'
Assert-FileContains -RelativePath 'scripts/test_merge_section.py' -Pattern 'test_final_output_exact_limit_is_written_and_idempotent' -Description 'exact output-limit closure regression'
...
Assert-FileContains -RelativePath 'scripts/test_merge_section.py' -Pattern 'test_raw_newline_budget_accepts_exact_limit_and_rejects_limit_plus_one' -Description 'newline exact and limit-plus-one regression'
Assert-FileContains -RelativePath 'scripts/test_merge_section.py' -Pattern 'test_cli_rejects_target_newline_limit_plus_one_before_decode_or_write' -Description 'target newline pre-decode CLI regression'
Assert-FileContains -RelativePath 'scripts/test_merge_section.py' -Pattern 'test_cli_rejects_block_newline_limit_plus_one_before_merge_or_write' -Description 'block newline pre-merge CLI regression'
Assert-FileContains -RelativePath 'scripts/test_merge_section.py' -Pattern 'test_output_newline_limit_rejects_growth_without_writing' -Description 'output newline no-write regression'
Assert-FileContains -RelativePath 'scripts/test_merge_section.py' -Pattern 'test_exact_newline_limits_preserve_lf_crlf_bom_and_second_run' -Description 'LF CRLF BOM newline idempotency regression'
Assert-FileContains -RelativePath 'scripts/test_merge_section.py' -Pattern '(?is)representative\s*=.*\*\s*100_000.*_MAX_TARGET_BYTES.*_MAX_TARGET_NEWLINES.*_assert_raw_newline_budget' -Description '100000-line ordinary paragraph compatibility regression'
Assert-FileContains -RelativePath 'scripts/test_merge_section.py' -Pattern 'test_default_measurement_matrix_stays_within_target_line_budget' -Description 'default peak matrix newline compatibility regression'
...
Assert-FileContains -RelativePath 'scripts/measure_peak_memory.py' -Pattern '(?is)DEFAULT_TARGET_BYTES\s*=\s*1 \* 1024 \* 1024.*MAX_TARGET_BYTES\s*=\s*8 \* 1024 \* 1024.*_bounded_int\("target-bytes", MIN_TARGET_BYTES, MAX_TARGET_BYTES\)' -Description 'current default and explicit maximum peak-matrix sizes'
Assert-FileContains -RelativePath 'docs/peak-memory-characterization.md' -Pattern '(?is)commit `da8991d`.*historical evidence.*既定.*1 MiB.*byte上限.*8 MiB.*dense 8 MiB.*意図どおり拒否' -Description 'historical 8 MiB evidence and current 1 MiB default contract'
